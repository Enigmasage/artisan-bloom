...
import { Plus, Edit, Trash2, Eye, ImageIcon, X } from "lucide-react";
...
const SellerAwareness = () => {
  ...
  const isOwnArticle = (article: AwarenessArticle) =>
    article.authorId === user?.id && article.authorRole === "seller";
  ...
  const handleAddArticle = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newArticle: AwarenessArticle = {
      ...
      authorId: user?.id,
      authorRole: "seller",
      authorName: user?.name || "Seller",
    };
    setArticles([newArticle, ...articles]);
    setIsAddDialogOpen(false);
    setImagePreview(null);
    toast({ title: "Article Published", description: "Your article is now live on the awareness page." });
  };

  const deleteArticle = (articleId: string) => {
    const article = articles.find((a) => a.id === articleId);
    if (!article || !isOwnArticle(article)) {
      toast({ title: "Permission Denied", description: "You can only delete your own articles.", variant: "destructive" });
      return;
    }
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Create and manage your awareness content</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {articles.map((article) => {
            const owned = isOwnArticle(article);
            return (
              <Card key={article.id} className="heritage-card overflow-hidden">
                ...
                <CardContent className="p-4">
                  ...
                  <div className="flex gap-2 mt-4 pt-4 border-t">
                    <Link to={`/awareness/${article.id}`} className={owned ? "flex-1" : "w-full"}>
